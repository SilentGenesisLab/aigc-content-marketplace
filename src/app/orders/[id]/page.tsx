...
export default async function OrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const order = await prisma.order.findUnique({
    ...
    include: {
      ...
      subOrders: { include: { creator: { select: { name: true } }, payment: true } },
    },
  });
  if (!order) notFound();
    ...
    <div className="content">
      ...
      <div className="detail-grid" style={{ marginTop: 24 }}>
        <div className="grid">
          ...
          {order.clientId === user.id && order.applications.length > 0 && ["OPEN", "SELECTING"].includes(order.status) && <section className="card"><h2>报名者对比与中选</h2><p className="muted">仅展示真实作品和履约数据。最多中选 {order.mode === "CONTEST" ? 1 : order.slots} 人。</p>{order.applications.map((application) => <div key={application.id} style={{ padding: "14px 0", borderBottom: "1px solid var(--line)" }}><div className="row between"><Link href={`/creators/${application.creatorId}`} style={{color:"var(--blue)"}}><strong>{application.creator.name}</strong></Link><span className="chip">已完成 {application.creator._count.subOrders} 单</span></div><p>{application.pitch}</p><div className="row wrap"><span className="chip orange">报价 ¥{Number(application.quoteAmount||order.budgetAmount||0).toFixed(2)}</span><span className="chip">预计 {application.promisedDays||"—"} 天</span><span className="chip green">预计到手 ¥{(Number(application.quoteAmount||order.budgetAmount||0)*0.98).toFixed(2)}</span></div><p className="muted">{application.creator.creatorProfile?.bio||"创作者尚未填写介绍"}</p><div className="muted" style={{ fontSize: 13 }}>{application.creator.creatorProfile?.skills.join(" · ") || "尚未完善技能"} · {application.creator.reviewsReceived.length ? `${(application.creator.reviewsReceived.reduce((sum, review) => sum + review.rating, 0) / application.creator.reviewsReceived.length).toFixed(1)} 分` : "暂无评价"}</div>{application.creator.portfolios.length?<div className="row wrap" style={{marginTop:10}}>{application.creator.portfolios.map(work=><a className="chip blue" href={work.fileUrl} target="_blank" rel="noreferrer" key={work.id}>{work.title}</a>)}</div>:<div className="muted" style={{fontSize:13,marginTop:8}}>暂无公开作品</div>}</div>)}<div style={{ marginTop: 16 }}><ApplicantSelector orderId={order.id} max={order.mode === "CONTEST" ? 1 : order.slots} applications={activeApplications.map((item) => ({ id: item.id, name: item.creator.name }))}/></div></section>}
          {order.subOrders.length > 0 && <section className="card"><h2>履约子订单</h2><div className="divider"/>{order.subOrders.map((subOrder) => <Link href="/work" className="row between" key={subOrder.id} style={{ padding: "10px 0" }}><span><strong>{subOrder.code}</strong> · {subOrder.creator.name}</span><span className={`chip ${subOrder.payment?.status==="FUNDED"?"green":"blue"}`}>{subOrder.payment?.status==="FUNDED"?"已托管（沙箱）":subOrder.status}</span></Link>)}</section>}
        </div>
        <aside className="grid" style={{ alignContent: "start" }}><div className="card"><h2>交付约束</h2><div className="divider"/><p><strong>{order.quantity}</strong> 条，单条 {order.duration}</p><p><strong>预算 ¥{Number(order.budgetAmount||0).toFixed(2)}</strong></p><p>截止：{order.deadline.toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" })}</p><p>名额：{order.slots} · 已报名 {order.applications.length}</p><p>包含修改 {order.maxRevisions} 次</p><p>{order.sampleRequired ? "报名需要样稿" : "报名无需样稿"}</p></div>{user.role === "CREATOR" && order.status === "OPEN" && !mine && <div className="card"><h2>参与创作</h2><p className="muted">确认你理解订单模式和交付约束后再报名。</p><ApplyForm orderId={order.id} budget={Number(order.budgetAmount||0)}/></div>}{mine && <div className="notice">你已报名该订单，可在“我的制作”跟踪中选结果。</div>}</aside>
      </div>
    </div>
  </AppShell>;
}
