    ...
    <div className="content">
      ...
      <div className="detail-grid" style={{ marginTop: 24 }}>
        <div className="grid">
          ...
          {order.clientId === user.id && order.applications.length > 0 && ["OPEN", "SELECTING"].includes(order.status) && <section className="card"><h2>报名者对比与中选</h2><p className="muted">仅展示真实作品和履约数据。最多中选 {order.mode === "CONTEST" ? 1 : order.slots} 人。</p>{order.applications.map((application) => <div key={application.id} style={{ padding: "14px 0", borderBottom: "1px solid var(--line)" }}><div className="row between"><strong>{application.creator.name}</strong><span className="chip">已完成 {application.creator._count.subOrders} 单</span></div><p>{application.pitch}</p><div className="muted" style={{ fontSize: 13 }}>{application.creator.creatorProfile?.skills.join(" · ") || "尚未完善技能"} · {application.creator.reviewsReceived.length ? `${(application.creator.reviewsReceived.reduce((sum, review) => sum + review.rating, 0) / application.creator.reviewsReceived.length).toFixed(1)} 分` : "暂无评价"}</div></div>)}<div style={{ marginTop: 16 }}><ApplicantSelector orderId={order.id} max={order.mode === "CONTEST" ? 1 : order.slots} applications={activeApplications.map((item) => ({ id: item.id, name: item.creator.name }))}/></div></section>}
