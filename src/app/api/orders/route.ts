...
export async function POST(request: Request) {
  try {
    const user = await requireUser([UserRole.CLIENT, UserRole.ADMIN]);
    ...
    const order = await prisma.order.create({ data: {
      ...
      acceptanceCriteria: required(data.acceptanceCriteria, "验收标准"), mode: data.mode === "CONTEST" ? OrderMode.CONTEST : OrderMode.MULTI_DELIVERY, slots, sampleRequired: Boolean(data.sampleRequired),
    } });
