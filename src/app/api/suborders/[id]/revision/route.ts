...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const previousRevisions = await prisma.revisionRequest.count({ where: { delivery: { subOrderId: id } } });
    if (previousRevisions >= 1) throw new ApiError("首周试运营每个子订单仅支持一次结构化修改，请进入争议流程");
